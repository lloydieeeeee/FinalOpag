...
function setFilter(f){cf=f;['All','Enabled','Disabled'].forEach(x=>document.getElementById('f'+x).classList.toggle('on',f===x.toLowerCase()));filterRows();}
function filterRows(){const q=document.getElementById('si').value.toLowerCase();document.querySelectorAll('.ltr').forEach(r=>r.style.display=(r.dataset.name.includes(q)&&(cf==='all'||r.dataset.status===cf))?'':'none');}
const sd={};
function sortBy(col){sd[col]=!sd[col];const tb=document.getElementById('ltBody');[...tb.querySelectorAll('.ltr')].sort((a,b)=>{const va=col==='name'?a.dataset.name:a.dataset.status,vb=col==='name'?b.dataset.name:b.dataset.status;return sd[col]?va.localeCompare(vb):vb.localeCompare(va);}).forEach(r=>tb.appendChild(r));}
function toggleAll(cb){document.querySelectorAll('.rchk').forEach(c=>{if(c.closest('tr').style.display!=='none')c.checked=cb.checked;});onChk();}
