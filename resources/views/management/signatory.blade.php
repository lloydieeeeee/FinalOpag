@extends('layouts.app')
@section('title', 'Management Settings — Signatory')
...
.role-badge{display:inline-block;padding:2px 10px;border-radius:20px;font-size:11px;font-weight:700;background:#f0fdf4;color:#15803d;}
.sig-preview{display:flex;flex-direction:column;align-items:center;gap:2px;}
.sig-preview-name{font-size:12px;font-weight:700;color:#111827;text-decoration:underline;}
.sig-preview-title{font-size:11px;color:#6b7280;}
...
.mc{background:#fff;border-radius:20px;padding:32px;width:480px;max-width:94vw;box-shadow:0 24px 64px rgba(0,0,0,.2);transform:scale(.93) translateY(10px);transition:transform .26s cubic-bezier(.34,1.56,.64,1);}.ov.show .mc{transform:scale(1) translateY(0);}
...
<div class="mgmt-page">
    ...
    <div class="sub-tab-bar">
        ...
        <a href="{{ route('settings.signatory') }}"       class="sub-tab-btn active">Signatory</a>
    ...
    <div class="mgmt-card">
        <div class="ch">
            <div>
                <p class="ct">Signatory</p>
                <p class="cd">Manage names and titles that appear on the CS Form 6 PDF — changes reflect immediately on all printed forms</p>
            </div>
            ...
            <table class="dt">
                <thead>
                    <tr>
                        ...
                        <th>Role / Section</th>
                        <th>Full Name <span style="font-weight:400;color:#d1d5db;">(underlined on PDF)</span></th>
                ...
                <tbody id="tbody">
                    ...
                    <tr class="row"
                        data-id="{{ $opt->id }}"
                        data-label="{{ strtolower($opt->label ?? '') }} {{ strtolower($opt->full_name ?? '') }}">
                        <td style="color:#9ca3af;font-family:monospace;font-size:12px;">{{ str_pad($i+1,2,'0',STR_PAD_LEFT) }}</td>
                        <td><span class="role-badge">{{ $opt->label }}</span></td>
                        <td style="font-weight:700;color:#111827;">{{ $opt->full_name ?? '—' }}</td>
                        <td style="color:#6b7280;font-size:12px;">{{ $opt->title ?? '—' }}</td>
                        <td>
                            @if($opt->full_name)
                            <div class="sig-preview">
                                <span class="sig-preview-name">{{ strtoupper($opt->full_name) }}</span>
                                <span class="sig-preview-title">{{ $opt->title }}</span>
                            </div>
                            @else
                            <span style="color:#d1d5db;font-size:12px;">— not set —</span>
                            @endif
                        </td>
                        <td>
                            <div class="ac">
                                <button class="ib e" title="Edit"
                                    onclick="openEdit({{ $opt->id }},'{{ addslashes($opt->label ?? '') }}','{{ addslashes($opt->full_name ?? '') }}','{{ addslashes($opt->title ?? '') }}')">
                                    <svg width="15" height="15" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </button>
                                <button class="ib d" title="Delete"
                                    onclick="openDel({{ $opt->id }},'{{ addslashes($opt->label ?? '') }}')">
                    ...
                    @empty
                    <tr><td colspan="6" style="padding:48px;text-align:center;color:#9ca3af;font-size:13px;">No signatories yet. Add one to have names appear on the PDF.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        <div class="mgmt-footer">{{ count($options) }} signatory(ies) · Changes reflect immediately on all CS Form 6 PDFs</div>
    </div>
</div>

<div id="fo" class="ov" onclick="if(event.target===this)closeForm()">
    <div class="mc">
        <h3 class="mt" id="mT">Add Signatory</h3>
        <p class="ms">This person's name and title will appear on the CS Form 6 PDF signature block.</p>
        <div class="fr">
            <label class="fl">Role / Section Label *</label>
            <input class="fi" id="fL" type="text" placeholder="e.g. HRMO Officer, Acting Governor">
            <p class="hint">Identifies which signature block this is on the form.</p>
        </div>
        <div class="fr">
            <label class="fl">Full Name *</label>
            <input class="fi" id="fN" type="text" placeholder="e.g. ENGR. ALMIRANTE A. ABAD" oninput="updatePreview()">
            <p class="hint">Shown underlined in the signature block.</p>
        </div>
        <div class="fr">
            <label class="fl">Title / Position *</label>
            <input class="fi" id="fT" type="text" placeholder="e.g. Provincial Agriculturist" oninput="updatePreview()">
            <p class="hint">Shown below the underlined name.</p>
        </div>
        <div style="background:#f8f9fa;border:1px solid #e5e7eb;border-radius:10px;padding:16px;text-align:center;margin-top:4px;">
            <p style="font-size:10px;color:#9ca3af;margin-bottom:8px;text-transform:uppercase;letter-spacing:.06em;font-weight:700;">PDF Preview</p>
            <div id="previewName" style="font-family:'Times New Roman',serif;font-size:14px;font-weight:700;text-decoration:underline;color:#111827;">—</div>
            <div id="previewTitle" style="font-family:'Times New Roman',serif;font-size:11px;color:#6b7280;margin-top:2px;">—</div>
        </div>
        <div class="mf">
            <button class="bcm" onclick="closeForm()">Cancel</button>
            <button class="bsm" id="bS" onclick="saveForm()">Add Signatory</button>
        </div>
    </div>
</div>
...
<div id="do" class="ov" onclick="if(event.target===this)closeDel()">
    <div class="mc" style="width:400px;">
        ...
        <h3 class="mt" style="margin-bottom:8px;">Delete Signatory?</h3>
        <p style="font-size:13px;color:#6b7280;margin:0 0 6px;">You are about to delete <strong id="dN"></strong>.</p>
        <p style="font-size:12px;color:#ef4444;margin:0 0 24px;">This will remove them from the PDF signature block.</p>
...
const BASE = '{{ route("settings.signatory") }}';
let eid = null, did = null;

function filterRows() {
    const q = document.getElementById('si').value.toLowerCase();
    document.querySelectorAll('.row').forEach(r => r.style.display = r.dataset.label.includes(q) ? '' : 'none');
}
function updatePreview() {
    document.getElementById('previewName').textContent  = document.getElementById('fN').value.trim().toUpperCase() || '—';
    document.getElementById('previewTitle').textContent = document.getElementById('fT').value.trim() || '—';
}
function openAdd() {
    eid = null;
    document.getElementById('mT').textContent = 'Add Signatory';
    document.getElementById('bS').textContent = 'Add Signatory';
    ['fL','fN','fT'].forEach(id => document.getElementById(id).value = '');
    updatePreview();
    document.getElementById('fo').classList.add('show');
    setTimeout(() => document.getElementById('fL').focus(), 150);
}
function openEdit(id, label, fullName, title) {
    eid = id;
    document.getElementById('mT').textContent = 'Edit Signatory';
    document.getElementById('bS').textContent = 'Save Changes';
    document.getElementById('fL').value = label;
    document.getElementById('fN').value = fullName;
    document.getElementById('fT').value = title;
    updatePreview();
    document.getElementById('fo').classList.add('show');
    setTimeout(() => document.getElementById('fL').focus(), 150);
}
...
function saveForm() {
    const label = document.getElementById('fL').value.trim();
    const fullName = document.getElementById('fN').value.trim();
    const title = document.getElementById('fT').value.trim();
    if (!label)    { toast('Error','Role label is required.','error'); return; }
    if (!fullName) { toast('Error','Full name is required.','error'); return; }
    if (!title)    { toast('Error','Title is required.','error'); return; }
    ...
    fetch(eid ? `${BASE}/${eid}` : BASE, {
        ...
        body: JSON.stringify({label, full_name: fullName, title}),
    }).then(r=>r.json()).then(d=>{
        btn.disabled = false;
        btn.textContent = eid ? 'Save Changes' : 'Add Signatory';
...
function execDel() {
    if (!did) return;
    fetch(`${BASE}/${did}`,{method:'DELETE',headers:{'X-CSRF-TOKEN':CSRF,'X-Requested-With':'XMLHttpRequest'}})
    .then(r=>r.json()).then(d=>{ closeDel(); if(d.success){toast('Deleted',d.message,'error');setTimeout(()=>location.reload(),900);}else toast('Error',d.message,'error'); });
}
