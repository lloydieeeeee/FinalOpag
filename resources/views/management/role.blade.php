@extends('layouts.app')
@section('title', 'Management Settings — Role')
...
<div class="mgmt-page">
    ...
    <div class="sub-tab-bar">
        ...
        <a href="{{ route('settings.role') }}"            class="sub-tab-btn active">Role</a>
    </div>
    ...
    <div class="mgmt-card">
        <div class="ch">
            <div>
                <p class="ct">Role</p>
                <p class="cd">Manage role options for the leave application form</p>
            </div>
        ...
        <div class="tsa">
            <table class="dt">
                ...
                <tbody id="tbody">
                    ...
                    @empty
                    <tr><td colspan="3" style="padding:48px;text-align:center;color:#9ca3af;font-size:13px;">No role options yet.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
...
<div id="fo" class="ov" onclick="if(event.target===this)closeForm()">
    <div class="mc">
        <h3 class="mt" id="mT">Add Role Option</h3>
        <p  class="ms" id="mS">Enter the option label below.</p>
        <div class="fr"><label class="fl">Label *</label><input class="fi" id="fN" type="text" placeholder="e.g. Department Head"></div>
        <div class="mf">
...
<script>
const CSRF='{{csrf_token()}}', BASE='{{route("settings.role")}}';
...
function openAdd(){eid=null;document.getElementById('mT').textContent='Add Role Option';document.getElementById('bS').textContent='Add Option';document.getElementById('fN').value='';document.getElementById('fo').classList.add('show');setTimeout(()=>document.getElementById('fN').focus(),150);}
