@extends('layouts.app')
@section('title', 'Management Settings — Position')
...
.sw input{padding:8px 12px 8px 32px;font-size:13px;border:1.5px solid #e5e7eb;border-radius:9px;outline:none;width:230px;transition:border-color .15s;color:#374151;}.sw input:focus{border-color:#2d5a1b;}
...
.mc{background:#fff;border-radius:20px;padding:32px;width:460px;max-width:94vw;box-shadow:0 24px 64px rgba(0,0,0,.2);transform:scale(.93) translateY(10px);transition:transform .26s cubic-bezier(.34,1.56,.64,1);}.ov.show .mc{transform:scale(1) translateY(0);}
...
.fr2{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:16px;}.mf{display:flex;justify-content:flex-end;gap:10px;margin-top:26px;}
...
<div class="mgmt-page">
    ...
    <div class="sub-tab-bar">
        ...
        <a href="{{ route('settings.position') }}"        class="sub-tab-btn active">Position</a>
    ...
    <div class="mgmt-card">
        ...
        <div class="ch">
            <div><p class="ct">Position</p><p class="cd">Manage employee positions and their codes</p></div>
            <div style="display:flex;align-items:center;gap:10px;">
                <div class="sw">
                    ...
                    <input type="text" id="si" placeholder="Search position or code..." oninput="filterRows()">
                </div>
        ...
        <div class="tsa">
            <table class="dt">
                ...
                <tbody id="pBody">
                    ...
                    <tr class="pr" data-id="{{ $p->position_id }}" data-name="{{ strtolower($p->position_name) }}" data-code="{{ strtolower($p->position_code) }}">
                        <td style="color:#9ca3af;font-family:monospace;font-size:12px;">{{ str_pad($i+1,3,'0',STR_PAD_LEFT) }}</td>
                        ...
                        <td>
                            <div class="ac">
                                <button class="ib e" title="Edit" onclick="openEdit({{ $p->position_id }},'{{ addslashes($p->position_name) }}','{{ $p->position_code }}')">
                                    <svg width="15" height="15" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </button>
                                <button class="ib d" title="Delete" onclick="openDel({{ $p->position_id }},'{{ addslashes($p->position_name) }}')">
                    ...
                    @empty
                    <tr><td colspan="5" style="padding:52px;text-align:center;color:#9ca3af;font-size:13px;">No positions found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>

        <div class="mgmt-footer">{{ count($positions) }} position(s)</div>
    </div>
...
<div id="fo" class="ov" onclick="if(event.target===this)closeForm()">
    <div class="mc">
        <h3 class="mt" id="mT">Add Position</h3>
        <p  class="ms" id="mS">Fill in the position details below.</p>
        <div class="fr2">
            <div>
                <label class="fl">Position Name *</label>
                <input class="fi" id="fN" type="text" placeholder="e.g. Agriculturist II">
            </div>
            <div>
                <label class="fl">Position Code *</label>
                <input class="fi" id="fC" type="text" placeholder="e.g. AG II" style="text-transform:uppercase;">
            </div>
        </div>
        <div class="mf">
            <button class="bcm" onclick="closeForm()">Cancel</button>
            <button class="bsm" id="bS" onclick="saveForm()">Add Position</button>
        </div>
    </div>
</div>
...
<div id="do" class="ov" onclick="if(event.target===this)closeDel()">
    <div class="mc" style="width:380px;">
        ...
        <h3 class="mt" style="margin-bottom:8px;">Delete Position?</h3>
        <p style="font-size:13px;color:#6b7280;margin:0 0 24px;">You are about to delete <strong id="dN"></strong>. Employees holding this position may be affected.</p>
...
<script>
const CSRF='{{csrf_token()}}',BASE='{{route("settings.position")}}';
let eid=null,did=null;
function filterRows(){const q=document.getElementById('si').value.toLowerCase();document.querySelectorAll('.pr').forEach(r=>r.style.display=(r.dataset.name.includes(q)||r.dataset.code.includes(q))?'':'none');}
...
function openAdd(){eid=null;document.getElementById('mT').textContent='Add Position';document.getElementById('mS').textContent='Fill in the position details below.';document.getElementById('bS').textContent='Add Position';document.getElementById('fN').value='';document.getElementById('fC').value='';document.getElementById('fo').classList.add('show');setTimeout(()=>document.getElementById('fN').focus(),150);}
function openEdit(id,nm,cd){eid=id;document.getElementById('mT').textContent='Edit Position';document.getElementById('mS').textContent='Update the position details.';document.getElementById('bS').textContent='Save Changes';document.getElementById('fN').value=nm;document.getElementById('fC').value=cd;document.getElementById('fo').classList.add('show');}
function closeForm(){document.getElementById('fo').classList.remove('show');}
function saveForm(){const nm=document.getElementById('fN').value.trim(),cd=document.getElementById('fC').value.trim();if(!nm||!cd){toast('Error','Position name and code are required.','error');return;}const b=document.getElementById('bS');b.disabled=true;b.textContent='Saving…';fetch(eid?`${BASE}/${eid}`:BASE,{method:eid?'PUT':'POST',headers:{'X-CSRF-TOKEN':CSRF,'X-Requested-With':'XMLHttpRequest','Content-Type':'application/json'},body:JSON.stringify({position_name:nm,position_code:cd})}).then(r=>r.json()).then(d=>{b.disabled=false;b.textContent=eid?'Save Changes':'Add Position';if(d.success){closeForm();toast(eid?'Updated':'Added',d.message,'success');setTimeout(()=>location.reload(),900);}else toast('Error',d.message,'error');});}
